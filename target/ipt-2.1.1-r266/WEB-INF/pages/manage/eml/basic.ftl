...
<script type="text/javascript">
	...
	$(document).ready(function(){
		//New function
		<#if eml.description??>
			var bsc='${eml.description?url}';
      		bsc=decodeURIComponent(bsc).split('[abstract]');
      
			$("#eml\\.description").val(bsc[0]);
			$("#Abstract").val(bsc[1]);
		</#if>
      ...
      // Populate subtype list depending on core type selected
		...
		$("#save").click(function(event){
		      var des=jQuery("#eml\\.description").val();
		      var abst=jQuery("#Abstract").val();
		      if(des.indexOf("[abstract]")==-1){
		      	des=jQuery("#eml\\.description").val();
		      }else{
		      	des='';
		      }
		      if(abst.indexOf('[abstract]')==-1){
		      	abst=jQuery("#Abstract").val();
		      }else{
		      	abst='';
		      }
		      var total=des+'[abstract]'+abst;
		      total=total.replace(/'/g, '"');
		      if(total=='[abstract]'){
		      	$("#eml\\.description").val('');
		      }else{
		      	$("#eml\\.description").val(total);
		      }
		   });  
	});
...
<h2 class="subTitle"><@s.text name='manage.metadata.basic.title'/></h2>
    ...
    <p><@s.text name="manage.metadata.basic.required.message" /></p>
  	...
  	<@text name="Abstract" requiredField=false help="i18n" />
