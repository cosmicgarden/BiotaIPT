...
<script type="text/javascript" src="${baseURL}/js/jconfirmationI.jquery.js"></script>
<title><@s.text name='manage.metadata.additional.title'/></title>
<script type="text/javascript">
	$(document).ready(function(){
		<#if eml.additionalInfo??>
        	var adi='${eml.additionalInfo?url}';
          adi=decodeURIComponent(adi).split('[agradecimientos]');
        	var disct=adi[0].split('[discusion]');
          if(adi.length!=1){
        	 $("#eml\\.additionalInfo").val(disct[1]);
            $("#acknowledgements").val(adi[1]);
          }
    </#if>
    ...
    var control=true;
		...
		$('.confirm').jConfirmAction({question : "<@s.text name="eml.intellectualRights.licenses.confirmation"/>", yesAnswer : "<@s.text name="basic.yes"/>", cancelAnswer : "<@s.text name="basic.no"/>", isRigths: true });
    ...
    if(control && ($.trim(optionSelected)=='<@s.text name="eml.intellectualRights.nolicenses"/>')){
      $("#save").click(function(event){
            //event.preventDefault();
            var dics=jQuery("#eml\\.additionalInfo").val();
            var ackno=jQuery("#acknowledgements").val();
...
<h2 class="subTitle"><@s.text name='manage.metadata.additional.title.disc'/></h2>
    ...
    <div id="Additional-Metadata">
    	...
    	<@text name="eml.additionalInfo" i18nkey="eml.additionalInfo.dis" help="i18n"/>
        <@text name="acknowledgements" i18nkey="eml.additionalInfo.acknow" help="i18n"/>
  	</div>
